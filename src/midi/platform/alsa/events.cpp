...
events::Event makeEvent(const snd_seq_event_t* event, const snd_seq_t* handle)
{
    ...
    case SND_SEQ_EVENT_CLIENT_START:
        return events::ClientStart{.client = event->data.addr.client};

    case SND_SEQ_EVENT_CLIENT_EXIT:
        return events::ClientExit{.client = event->data.addr.client};

    case SND_SEQ_EVENT_CLIENT_CHANGE:
        return events::ClientChange{.client = event->data.addr.client};

    case SND_SEQ_EVENT_PORT_START:
        return events::PortStart{event->data.addr.client,
                                 event->data.addr.port};

    case SND_SEQ_EVENT_PORT_EXIT:
        return events::PortExit{event->data.addr.client, event->data.addr.port};

    case SND_SEQ_EVENT_PORT_CHANGE:
        return events::PortChange{event->data.addr.client,
                                  event->data.addr.port};

    case SND_SEQ_EVENT_PORT_SUBSCRIBED:
        return events::PortSubscribed{};

    case SND_SEQ_EVENT_PORT_UNSUBSCRIBED:
        return events::PortUnsubscribed{};
...
snd_seq_event_t makeEvent(const events::Event& event)
{
    return std::visit(
        overloaded{[](const auto& event) {
                       // This overload includes Unknown, Bounce and all
                       // Client events as they will not be generated by this
                       // application.
